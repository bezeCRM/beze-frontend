...
const FormSchema = z
  .object({
    ...
    photoes: z.array(
      z.object({ id: z.string(), uri: z.string() })
    ).max(3, 'максимум 3 фото').optional(),
  })
  .superRefine((data, ctx) => {
    if (!data.category) {
      ctx.addIssue({ code: 'custom', path: ['category'], message: 'выберите категорию' })
    }
  })
...
export function useProductCreateForm(defaultCategory: Category | null = null) {
  ...
  function removeFilling(id: string) {
    const list: Filling[] = form.getValues('fillings') ?? []
    form.setValue('fillings', list.filter(f => f.id !== id))
  }
  ...
  function removeIngredient(id: string) {
    const list: Ingredient[] = form.getValues('ingredients') ?? []
    form.setValue('ingredients', list.filter(i => i.id !== id))
  }
  ...
  function removePhoto(id: string) {
    const list: Photo[] = form.getValues('photoes') ?? []
    form.setValue('photoes', list.filter(p => p.id !== id))
  }
